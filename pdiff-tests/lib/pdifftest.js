...
module.exports = {
  startTest: function(casper, relativePath, cb) {
    ...
    // This could be used to make Math.random() deterministic, for example:
    // casper.options.clientScripts = [];

    casper.start(relativePath, function() {
      this.viewport(1024, 768);  // default is 400x300, which is tiny!
  ...
  takeScreenshot: function(casper, opt_area) {
    // To capture a screenshot to a file, use this:
    actual_screenshot = 'data:image/png;base64,' + casper.captureBase64('png', opt_area);
